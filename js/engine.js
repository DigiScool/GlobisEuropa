	...
	var Engine = Class.extend({
		...
		setLevel: function(level,puzzle,callback){
			...
			document.onkeydown = function(event) {
				console.log(self.container_bg.x);
						...
						target.addEventListener("mousedown",function(evt) {
							...
							evt.onMouseUp = function(ev){
								...
								self.container_dndParts.alpha = 1;
  								...
  								// Versuche einen Hitshape zu finden
  								console.log('Container X: '+self.container_bg.x);
  								
  								if(self.level.stage == 1){

  									var image_x = dndPart.x + (dndPart.image.width / 2) + self.container_bg.x;
  									var image_y = dndPart.y + (dndPart.image.height / 2);
  								}
  								
  								if(self.level.stage == 2){
  									var image_x = ev.stageX;
  									var image_y = ev.stageY;
  								}
  								
  								console.log('image_x: '+image_x+' : image_y: '+image_y);
  								console.log('Country-ID:' + dndPart.countryId);
  								
  								var hitShape = self.container_hitshapes.getObjectUnderPoint(image_x,image_y);
